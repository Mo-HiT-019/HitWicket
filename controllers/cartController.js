...
const { RAZ_SECRET_KEY } = process.env;

var instance = new Razorpay({
  key_id: "rzp_test_cWq8vJZULM22H7",
  key_secret: RAZ_SECRET_KEY,
});
  ...
  try {
    if (req.session.user) {
      ...
      if (usr.blocked === true) {
        ...
        req.flash(
          "errorMessage",
          `Sorry ${use.name},Your account has been blocked`
        );
        return res.redirect("/");
      }
    }
    ...
    for (let i = 0; i < user.cart.length; i++) {
      ...
      if (item.productId && item.productId.price !== undefined) {
        const priceToConsider =
          item.productId.offers && item.productId.offers.length > 0
...
const addToCart = async (req, res) => {
  try {
    ...
    const discountedPrice =
      product.offers && product.offers[0] && product.offers[0].discountedPrice
        ? product.offers[0].discountedPrice
        : product.price;

    const existingCartItem = user.cart.find(
      (cartItem) => cartItem.productId && cartItem.productId.equals(product._id)
    );
...
const renderCheckout = async (req, res) => {
  try {
    ...
    for (let i = 0; i < sellingPrice.length; i++) {
      totalAmount +=
        parseInt(sellingPrice[i].discountedPrice || sellingPrice[i].price, 10) *
  ...
  try {
    ...
    for (const cartItem of userData.cart) {
      ...
      if (remainingStock < 0) {
        return res.json({
          success: false,
          message: "Insufficient stock for one or more products in the cart",
          showAlert: true,
        });
      }
    ...
    if (req.body.coupen !== "") {
      ...
      for (let i = 0; i < cartList.length; i++) {
        items.push({
          ...
          price:
            parseInt(cartList[i].price) -
            (parseInt(cartList[i].price) * dicount) / 100,
          status: status,
        });
      }
    } else {
    ...
    if (req.body.payment_method === "COD") {
      ...
      if (createOrder) {
        ...
        await User.updateOne(
          { _id: user._id },
          { $set: { user_wallet: parseInt(user.user_wallet) - priceValue } }
        );
